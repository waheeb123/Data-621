...
$$
0<\text { Precision }=\frac{T P}{T P+F P}<1
$$
...
$$
0<\text { Sensitivity }=\frac{T P}{T P+F N}<1
$$

The F1 score is defined as:

$$
0<F 1 \text { Score }=\frac{2 \times \text { Precision } \times \text { Sensitivity }}{\text { Precision }+ \text { Sensitivity }}<1
$$
...
### 11. Produce a classification metrics Table
...
result <- tibble(
  Metrics = c("Accuracy", "Classification Error Rate", "Precision", "Sensitivity", "Specificity", "F1 Score"),
  Outputs = c(Accuracy(class_df), Classification_Error_Rate(class_df), Precision(class_df),
              Sensitivity(class_df), Specificity(class_df), F1_score(class_df))
) %>% 
  mutate(Outputs = round(Outputs, 4))

result %>%
  kable(caption = "Summary of Classification Output Values") %>%
